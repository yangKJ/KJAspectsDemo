...
@property(nonatomic,strong) NSArray *kj_arguments;
@end

@implementation KJAspectsTool
...
+ (id<AspectToken>)kj_AspectsWithClassName:(NSString*)class_name Selector:(SEL)selector Options:(KJAspectOptions)options UsingBlock:(void (^_Nullable)(KJAspectsTool *info, NSError *error))block{
    NSError *error = nil; /// 如果出现异常，则该值不为空
    /** NSClassFromString是一个很有用的东西,用此函数进行动态加载尝试,如果返回nil, 则不能加载此类的实例 */
    id metalClass = [[NSClassFromString(class_name) alloc] init];
    /** 也可以使用objc_getMataClass传入对应类的c字符串 */
//    id metalClass = objc_getMetaClass(class_name.UTF8String);
    if (metalClass == nil) return nil;
    id<AspectToken> aspect = [objc_getMetaClass(class_name.UTF8String) aspect_hookSelector:selector withOptions:(AspectOptions)options usingBlock:^(id <AspectInfo> info){
        KJAspectsTool *infooo = [KJAspectsTool new];
        infooo.kj_instance = info.instance;
        infooo.kj_originalInvocation = info.originalInvocation;
        infooo.kj_arguments = info.arguments;
        /// 回调出去处理数据
        !block?:block(infooo,error);
    } error:&error];
    
    return aspect; /// 返回服从AspectToken协议的对象，可以进行移除等操作  [aspect remove];
}
...
+ (id<AspectToken>)kj_AspectsWithMetalClass:(id)metalClass Selector:(SEL)selector Options:(KJAspectOptions)options UsingBlock:(void (^_Nullable)(KJAspectsTool *info, NSError *error))block{
    NSError *error = nil; /// 如果出现异常，则该值不为空
    return [metalClass aspect_hookSelector:selector withOptions:(AspectOptions)options usingBlock:^(id <AspectInfo> info){
        KJAspectsTool *infooo = [KJAspectsTool new];
        infooo.kj_instance = info.instance;
        infooo.kj_originalInvocation = info.originalInvocation;
        infooo.kj_arguments = info.arguments;
